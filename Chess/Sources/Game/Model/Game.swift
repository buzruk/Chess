...
extension Game {
  ...
  var state: GameState {
    ...
    let canMove = allMoves(for: color).contains { move in
      var newBoard = self
  ...
  func canMove(from: Position, to: Position) -> Bool {
    ...
    switch this.type {
    case .pawn: return move(piece: this, for: .pawn, from: from, to: to, delta: delta)
    case .rook: return move(piece: this, for: .rook, from: from, to: to, delta: delta)
    case .knight: return move(piece: this, for: .knight, from: from, to: to, delta: delta)
    case .bishop: return move(piece: this, for: .bishop, from: from, to: to, delta: delta)
    case .queen: return move(piece: this, for: .queen, from: from, to: to, delta: delta)
    case .king: return move(piece: this, for: .king, from: from, to: to, delta: delta)
    }
...
//    }
  ...
  func positionIsThreatened(_ position: Position, by color: ChessColor) -> Bool {
    return board.allPieces.contains { from, piece in
  ...
  func kingIsInCheck(for color: ChessColor) -> Bool {
  ...
  func canPromotePiece(at position: Position) -> Bool {
    guard let pawn = board.piece(at: position), pawn.type == .pawn,
...
//    if let pawn = board.piece(at: position),
//       pawn.type == .pawn,
//       (pawn.color == .white && position.y == 0) ||
//       (pawn.color == .black && position.y == 7)
//    {
...
//    return false
  ...
  func nextMove(for color: ChessColor) -> Move? {
    var bestMove: Move?
...
private extension Game {
  func allMoves(for color: ChessColor) ->
  ...
  func allThreats(for color: ChessColor) -> LazyFilterSequence<[(position: Position, piece: Piece)]> {
  ...
  func pawnCanTake(from: Position, with delta: Delta) -> Bool {
    guard abs(delta.x) == 1, let pawn = board.piece(at: from)
    else { return false }

    assert(pawn.type == .pawn)

    switch pawn.color {
    case .white: return delta.y == -1
    case .black: return delta.y == 1
    }
  }
  ...
  func castlingPermitted(from: Position, to: Position) -> Bool {
    ...
    if pieceHasMoved(at: kingPosition) { return false }

    let isKingSide = (to.x == 6)
    let rookPosition = Position(x: isKingSide ? 7 : 0, y: kingsRow)

    if pieceHasMoved(at: rookPosition) { return false }

    return
      !(isKingSide ? 5 ... 6 : 1 ... 3).contains {
        board.piece(at: Position(x: $0, y: kingsRow)) != nil
      }

      &&
    
      !(isKingSide ? 4 ... 6 : 2 ... 4).contains {
